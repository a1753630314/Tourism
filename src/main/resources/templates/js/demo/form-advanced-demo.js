...
//              var imgd=$image.toDataURL('image/jpeg');
                 
                var base64Img = $image.cropper("getDataURL");
//              var ca=convertImageToCanvas(base64Img);
//              var simg=convertCanvasToImage(ca);
                var cropUrl = base64Img.replace(/png;/, "jpeg;")
                 $('.img-preview img').attr("src",cropUrl);  
//               .toDataURL("png")
			 ...
			 */
			function down() {
			    var sampleImage = $("#ringoImage img")[0],
			    canvas = convertImageToCanvas(sampleImage);
			    url = canvas.toDataURL("image/jpeg");//PNG格式
			    console.log(url)
			    //以下代码为下载此图片功能
			    var triggerDownload = $("#t").attr("href", url);
